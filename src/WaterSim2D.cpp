...
void WaterSim2D::setup(size_t sizeX, size_t sizeY, double dt, double dx, double dr, double rho, double gravity) {
    this->sizeX = sizeX;
    this->sizeY = sizeY;
    this->dt = dt;
    this->dx = mac.dx = dx;
    ...
    mac = MACGrid2D::create(sizeX, sizeY, dx);
    newMac = MACGrid2D::create(sizeX, sizeY, dx);
    p = Array2D<double>::create(sizeX, sizeY);
    cell = Array2D<CellType>::create(sizeX, sizeY);
    ...
    waterLevelSet = LevelSet::create(sizeX, sizeY, dx);
}
